%% Dual SMC-Controlled Quadcopter V-Configuration Tethered Payload
...
sim.dt    = 0.001; % Sim time step
sim.t_end = 25; % Total sim time
...
% Tether parameters (identical for both controllers)
...
smc_params.alt = struct('lambda', 6.0,  'eta', 5.0, 'phi', 0.5);
smc_params.lat = struct('lambda', 3.5,  'eta', 2.5, 'phi', 0.8);
smc_params.att = struct('lambda', 10.0, 'eta', 7.0, 'phi', 0.6);

controller1 = SMCQuadController(quad1, smc_params);
controller2 = SMCQuadController(quad2, smc_params);
...
fprintf('Final s_alt Q1/Q2: %.4f / %.4f\n', logger.s_alt1(end), logger.s_alt2(end));
fprintf('Final s_lat Q1/Q2: %.4f / %.4f\n', logger.s_lat1(end), logger.s_lat2(end));
...
figure('Position', [50 50 1400 900], 'Name', 'SMC Dual-Quad Tracking');
...
sgtitle('SMC Controller — Dual-Quad Tracking Performance', 'FontSize', 14, 'FontWeight', 'bold');

% Control inputs
figure('Position', [100 100 1200 500], 'Name', 'SMC Dual-Quad Control');
...
sgtitle('SMC Controller — Control Inputs', 'FontSize', 14, 'FontWeight', 'bold');

% Sliding surfaces
figure('Position', [150 100 1200 400], 'Name', 'SMC Sliding Surfaces');

subplot(1,3,1);
plot(t, logger.s_alt1, 'g-', 'LineWidth', 1.2); hold on;
plot(t, logger.s_alt2, 'm-', 'LineWidth', 1.2);
yline(0,'k--'); yline(smc_params.alt.phi,'r:','LineWidth',1); yline(-smc_params.alt.phi,'r:','LineWidth',1);
grid on; xlabel('Time [s]'); ylabel('s_z');
title('Altitude Sliding Surface');
legend('Quad 1','Quad 2','','Boundary','Location','best');

subplot(1,3,2);
plot(t, logger.s_lat1, 'g-', 'LineWidth', 1.2); hold on;
plot(t, logger.s_lat2, 'm-', 'LineWidth', 1.2);
yline(0,'k--'); yline(smc_params.lat.phi,'r:','LineWidth',1); yline(-smc_params.lat.phi,'r:','LineWidth',1);
grid on; xlabel('Time [s]'); ylabel('s_x');
title('Lateral Sliding Surface');

subplot(1,3,3);
plot(t, logger.s_alt1, 'g-', 'LineWidth', 1.2); hold on;
plot(t, logger.s_alt2, 'm-', 'LineWidth', 1.2);
yline(0,'k--'); yline(smc_params.att.phi,'r:','LineWidth',1); yline(-smc_params.att.phi,'r:','LineWidth',1);
grid on; xlabel('Time [s]'); ylabel('s_\theta');
title('Attitude Sliding Surface');

sgtitle('SMC Sliding Surface Convergence', 'FontSize', 14, 'FontWeight', 'bold');

% Trajectory 
figure('Position', [200 150 700 600], 'Name', 'SMC Dual-Quad Trajectory');
...
title('SMC — Flight Path (XZ Plane)', 'FontSize', 13);
legend('Quad 1','Quad 2','Payload','Waypoints','Ground','Location','best');

smc_metrics.rmse_z_mass  = rmse_z_mass;
smc_metrics.rmse_x_mass  = rmse_x_mass;
smc_metrics.ss_err_z     = ss_err_z;
smc_metrics.ss_err_x     = ss_err_x;
smc_metrics.max_err_z    = max_err_z;
smc_metrics.max_err_x    = max_err_x;
smc_metrics.total_effort = total_effort;
smc_metrics.TV_q1        = TV_q1;
smc_metrics.TV_q2        = TV_q2;
smc_metrics.max_tether1  = max(logger.tether1_force);
smc_metrics.max_tether2  = max(logger.tether2_force);
smc_metrics.max_pitch_q1 = max(abs(rad2deg(logger.theta1)));
smc_metrics.max_pitch_q2 = max(abs(rad2deg(logger.theta2)));
smc_metrics.rmse_z_q1    = rmse_z_q1;
smc_metrics.rmse_x_q1    = rmse_x_q1;
smc_metrics.rmse_z_q2    = rmse_z_q2;
smc_metrics.rmse_x_q2    = rmse_x_q2;

save('dual_smc_results.mat', 'logger', 'smc_metrics', 'waypoints', 'quad_params', 'mass_params', 'tether_params', 'formation_params', 'smc_params', 'sim');

fprintf('\nResults saved to dual_smc_results.mat\n');
